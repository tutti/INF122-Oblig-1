...
mineEgeneTester = TestList [
  ...
  TestCase (assertEqual "Stack test 1" (Number 11) (run "set emptyStack lambda n (case (n, 0)== -> 0, case otherwise -> emptyStack.); set push lambda stack (lambda value (case (set oldstack stack, set stack lambda n (case (n, 0)== -> value, case otherwise -> oldstack.))== -> 0, case otherwise -> value.)); set pop lambda stack (case (set underneath stack (1), set value stack (0))== -> 0, case (set stack underneath, 0) == -> 0, case otherwise -> value.); set myStack emptyStack; push (myStack) (5); push (myStack) (7); push (myStack) (11); pop (myStack);")),
  TestCase (assertEqual "Stack test 2" (Number 7) (run "set emptyStack lambda n (case (n, 0)== -> 0, case otherwise -> emptyStack.); set push lambda stack (lambda value (case (set oldstack stack, set stack lambda n (case (n, 0)== -> value, case otherwise -> oldstack.))== -> 0, case otherwise -> value.)); set pop lambda stack (case (set underneath stack (1), set value stack (0))== -> 0, case (set stack underneath, 0) == -> 0, case otherwise -> value.); set myStack emptyStack; push (myStack) (5); push (myStack) (7); push (myStack) (11); pop (myStack); pop (myStack);")),
  TestCase (assertEqual "Stack test 3" (Number 5) (run "set emptyStack lambda n (case (n, 0)== -> 0, case otherwise -> emptyStack.); set push lambda stack (lambda value (case (set oldstack stack, set stack lambda n (case (n, 0)== -> value, case otherwise -> oldstack.))== -> 0, case otherwise -> value.)); set pop lambda stack (case (set underneath stack (1), set value stack (0))== -> 0, case (set stack underneath, 0) == -> 0, case otherwise -> value.); set myStack emptyStack; push (myStack) (5); push (myStack) (7); push (myStack) (11); pop (myStack); pop (myStack); pop (myStack);"))]
